...
// 					';
	...
	public static function SearchCapa($pager)
	{
		//$fileID = HrLib::randomID(20).'.json';
		$editDel = "";
		$data = array();
		...
		$deleteLink = link_to('Delete', 'capa/supplierCapaDelete?id=' . $record->getId(),
		                    array('confirm' => 'Record [ '.$record->getReportNo() . ' ]  Sure to delete this record?')); 
		
		$editDel = $editLink .' | '.$deleteLink;
		$data[] = array(
				  'seq'=>'<small>'.$seq.'</small>'
				, 'action'=>'<small>'.$editDel.'</small>'
				, 'report_no'=> '<small>'.$record->getReportNo().'</small>'
				, 'title'=> '<small>'.$record->getTitle().'</small>'
				, 'reported_by'=> '<small>'.$record->getReportedBy().'</small>'
				, 'issue_date'=> '<small>'.$record->getIssueDate().'</small>'
				, 'follow_up'=> '<small>'.$record->getCarFollowupRequired().'</small>'
				, 'status'=> '<small>'.$record->getCarCaStatus().'</small>'
				, 'is_effective'=> '<small>'.$record->getCarPlanEffective().'</small>'
		);
		endforeach;
		return $data;
	}
	
	public static function SearchQAN($pager)
	{
		//$fileID = HrLib::randomID(20).'.json';
		$editDel = "";
		$data = array();
		...
		$data[] = array(
				...
				, 'reported_by'=> '<small>'.$record->getReportedBy().'</small>'
				, 'issue_date'=> '<small>'.$record->getIssueDate().'</small>'
				, 'reponse_date'=> '<small>'.$record->getResponseDate().'</small>'
				, 'follow_up'=> '<small>'.$record->getCarFollowupRequired().'</small>'
				, 'follow_date'=> '<small>'.$record->getCarFollowupDate().'</small>'
				, 'status'=> '<small>'.$record->getCarCaStatus().'</small>'
				, 'is_effective'=> '<small>'.$record->getCarPlanEffective().'</small>'
				, 'verified_by'=> '<small>'.$record->getCarVerifiedBy().'</small>'
				, 'car_closure_date'=> '<small>'.$record->getCarClosureDate().'</small>'
		);
		endforeach;
		return $data;
	}
