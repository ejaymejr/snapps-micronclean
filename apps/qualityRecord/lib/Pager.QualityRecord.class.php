...
class PagerQualityRecord
{
	...
	public static function ProcessLogPager($pager)
	{
		$editDel = "";
		$data = array();
		$seq = 0;
		$log = "
			  <script>
			  		ShowLog('Process Logs');
              </script>
              <a href='#' id='logWindow'>Log</a>
                        ";
		foreach ($pager as $record):
			$seq ++ ;
			$editUrl  = link_to('Edit', 'machine/processEventLogEdit?id='. $record->getId());
			$delUrl   = link_to('Delete', 'machine/processEventLogDelete?id='. $record->getId(),
                    	array('confirm' => 'Sure to delete this record?')); 
			$editDel = $editUrl . ' | ' . $delUrl ;
			$data[] = array(
					  'seq'=>'<small>'.$seq.'</small>'
					...
					, 'time'=> '<small>'.$record->getTransTime().'</small>'
					, 'machine'=> '<small>'.$record->getMachineNo().'</small>'
					, 'Failure'=> '<small>'.$record->getFailureMode().'</small>'
					, 'log'=> '<small>'.$log.'</small>'
			);
		endforeach;
		return $data;
