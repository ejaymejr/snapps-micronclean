 ...
 */
class nanocleanParticleAddAction extends SnappsAction
{
	var $preCount = 0;
	...
	public function preExecute()
	{
		...
		$id = $this->_G('id');
		$this->header = AirParticleCountNanoGPeer::retrieveByPK($id);
		$detail = false;
		if (!$this->header):
			$this->header = new AirParticleCountNanoG();
		...
		if ($this->getRequest()->getMethod() == sfRequest::POST )
		{
			...
			foreach($_POST as $k => $v):
				if (strpos($k, '_air_flow') > 0):
					...
					$detail = AirParticleCountNanoGCellPeer::GetDataByHeaderIDGridID($this->header->getId(), $gridID);
			  		if (! $detail):
			...
			$this->redirect('airparticle/nanocleanParticleEdit?id='. $this->header->getId() );
		}
	}
