 ...
 */
class dryerMonitoringAddAction extends SnappsAction
{
    var $preCount = 0;
    ...
    public function preExecute()
    {
        ...
        $id= $this->_G('id');
        $this->record = DryerMonitoringPeer::retrieveByPK($id);
        if (!$this->record)
        {
            $this->record = new DryerMonitoring();
    ...
    public function execute()
    {
        if ($this->getRequest()->getMethod() == sfRequest::POST)
        {
            ...
            $this->redirect('machine/dryerMonitoringSearch?HID[]=' . $this->record->getId());
        }
    }
