...
foreach ($pager as $record) {
    ...
    foreach ($plots as $pl) {
        if ($pl == 'Spec') :
        	for($x=0; $x<15; $x++) $plotDatas["Spec"]['values'][] = 40;
        	continue;
        endif;
         if ($pl == 'Up-Limit') :
        	for($x=0; $x<15; $x++) $plotDatas["Up-Limit"]['values'][] = 43;
        	continue;
        endif;
        if ($pl == 'Low-Limit') :
        	for($x=0; $x<15; $x++) $plotDatas["Low-Limit"]['values'][] = 37;
        	continue;
        endif;
...
foreach ($plotDatas as $name => $tmp) {
    ...
    if ($name == 'Spec') {    
    	$displayName = "SPEC: 40&deg;C";
    }
    if ($name == 'Up-Limit') {    
    	$displayName = "Upper Limit: 43&deg;C";
    }
    if ($name == 'Low-Limit') {    
    	$displayName = "Lower Limit: 37&deg;C";
    }
