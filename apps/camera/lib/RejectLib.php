...
class RejectLib
{
	...
	public static function EmailPhotos($fnameList, $sendTo = null, $emailDate = null, $emailToken)
	{
		...
		// 				$emailHistory = EmailHistoryPeer::GetDataByFileDetailId($fileDet->getId());
		// 				if ($emailHistory):
		// 					$emailDate  = $emailHistory->getEmailDate();
		// 					$emailToken = $emailHistory->getEmailToken();
		// 				endif;
		...
		$displayLocation = (sfConfig::get('http_intranet') . substr(url_for('photo/view'), 1 ) .'?token=' . $emailToken );
		//exit();
		$content =  $content.'
